...
public class BacteriologyResultValidator {
    ...
    public void validateFloraData(FloraDataDTO floraData, String testName, Errors errors) {
        if (floraData == null) {
            errors.rejectValue("floraData", "error.floraData.required",
                "Flora data is required for test: " + testName);
            return;
        }
        ...
        if (count != null && count > 0) {
            if (details == null || details.isEmpty()) {
                ...
                errors.rejectValue("floraData.details", "error.floraData.details.mismatch",
                    String.format("Number of flora details (%d) must match count (%d) for test: %s",
                        details.size(), count, testName));
            } else {
    ...
    private void validateFloraDetail(FloraDetailDTO detail, int floraNumber, String testName, Errors errors) {
        ...
        if (detail.getFloraNumber() == null) {
            ...
            errors.rejectValue(fieldPrefix + ".floraNumber", "error.floraDetail.floraNumber.mismatch",
                String.format("Flora number mismatch: expected %d but got %d for test: %s",
                    floraNumber, detail.getFloraNumber(), testName));
        }
    ...
    /**
     * Validate conditional test result
     * Ensures child tests are only filled when parent trigger condition is met
     *
     ...
     * @param errors Spring Errors object to accumulate validation errors
     */
    public void validateConditionalTestResult(
            String parentValue,
     ...
     * @param errors Spring Errors object to accumulate validation errors
     */
    public void validateSpecializedCultureResult(
            String cultureType,
        ...
        switch (cultureType.toUpperCase()) {
            case "NORMAL":
                if (cultureResult != null && !cultureResult.trim().isEmpty()) {
                    if (!cultureResult.equalsIgnoreCase("negative") && !cultureResult.equalsIgnoreCase("identification")) {
                        errors.rejectValue("cultureResult", "error.cultureResult.invalidForNormal",
                            String.format("Invalid culture result '%s' for NORMAL culture. Valid values: negative, identification",
                                cultureResult));
                    }
                }
                break;
            ...
            case "NEISSERIA_GONORRHOEAE":
                if (cultureResult != null && !cultureResult.trim().isEmpty()) {
                    if (!cultureResult.equalsIgnoreCase("negative") && !cultureResult.equalsIgnoreCase("neisseria_gonorrhoeae")) {
                        errors.rejectValue("cultureResult", "error.cultureResult.invalidForNeisseria",
                            String.format("Invalid culture result '%s' for NEISSERIA_GONORRHOEAE culture. Valid values: negative, neisseria_gonorrhoeae",
                                cultureResult));
                    }
                }
                break;
     ...
     * @param errors Spring Errors object to accumulate validation errors
     */
    public void validateFloraCountTest(
            Boolean isFloraCountTest,
            FloraDataDTO floraData,
            String testName,
            Errors errors) {
